...
Several specializations of the general \emph{is setting for} relation exist that can be used to distinguish between entities based on their type -- these are, among others, \emph{dul:includesAction}, \emph{dul:includesObject} and \emph{dul:includesAgent}.
However, assuming hierachical organization of objects (i.e. all objects are part of some map), and events (i.e. an event is composed into sub-events) only those entities at the root of the composition (e.g. the map) are required to be included explicitely in the episode. In particular, it is recommended that a NEEM narrative contain one \emph{Episode} individual, which represents the narrative as a whole, and that this individual be connected to top-level actions via the \emph{dul:includesAction} property.

It is also strongly recommended to include timing information about the narrative episode. To do this, the individual episode representing the narrative as a whole must connect via the \emph{dul:includesTime} property to a \emph{TimeInterval} individual. The \emph{TimeInterval} individual will connect via \emph{soma:hasIntervalBegin} and \emph{soma:hasIntervalEnd} to two floats representing the beginning and ending timestamps.

Typically, among the entities that an episode includes are also recordings of the episode. In such a case, the episode individual would connect via the \emph{soma:includesRecord} property to perhaps several \emph{InformationObject} individuals, each of these corresponding to a recording. In this way, multi-modal recording of an episode is supported. Each \emph{InformationObject} individual must have a data property \emph{soma:hasPersistentIdentifier} with a string value representing an address from which the recording data can be retrieved. The kind of recorded data can be indicated via the class of the recording individual, which should be a subclass of \emph{InformationObject}, e.g. \emph{VideoData}, and by a data property \emph{soma:hasDataFormat} that associates a string description -- usually a format name -- to the recording individual.
