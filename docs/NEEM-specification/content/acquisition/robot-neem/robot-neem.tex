...
Before you can begin to record your own \neems, you need to include the "cram-cloud-logger" package \todo{Abhijit: as an outsider I do not know, where can one find this package?} into your \cram plan.
...
The only things left to do start the logging before the plan execution and after the execution to finishing it. \todo{Abhijit: this sentence needs to be rewritten}
...
How to extend the \soma ontology is described in Section \ref{sec:extension} \todo{Abhijit: Are we going to have such section in the scope of this document?}.
...
Unknown tasks \todo{Abhijit: By unknown means the task that are not defined in the ontology or something else?} will be logged as \textit{PhysicalTask}.
...
where \textless CRAM-ACTION-NAME\textgreater is the name of your action used in the cram plan and \textless SOMA-ONTOLOGY-NAME\textgreater \todo{Abhijit: something is missing here in this sentence? i.e. what is soma-ontology-name: it is the action name mentioned in soma ontology?}. 
...
After you added your failure to the ontology, \todo{Abhijit: AFAIK, as of now there is no such failure concept in SOMA, am I missing something here?}open the "failure-handler.lisp" in the \cramloggerpackage package and add your new failure in the format:
...
After the \neem generation, the data will be stored in the created \neem folder under the file "roslog/<rostopic>.bson" \todo{Abhijit: text is not formatted correctly}.
