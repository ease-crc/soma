...
\begin{ODP}{Action Execution}
	...
	\begin{tikzpicture}
	    ...
	    \node[owlclass,below=0.6cm of EVT] (B) {Agent};
	...
	%% Example KnowRob language expressions
	\ODPEXAMPLES{
		\emph{is\_executed\_by('ACT','AG')} &
		\emph{ACT} is executed by \emph{AG} \\
		% % % % %
		\emph{is\_executed\_by('ACT',X)} &
		what are the agents that execute \emph{ACT}? \\
		% % % % %
		\emph{is\_executed\_by(X,'AG')} &
		which actions are executed by \emph{AG}?
	}
\end{ODP}
...
% % % % % % % % % % % % % % % % %
% % % % % % % % % % % % % % % % %
\section{Conceptual Classification}
