...
\begin{ODP}{Participation}
	...
	\ODPDEFINEDIN{DUL.owl}
	\ODPQUESTION{Which objects do participate in this event? Which events do this object participate in?}
	\ODPGRAPHIC{\begin{tikzpicture}
	    \node[owlclass] (EVT) {Event};
	...
	%% Example KnowRob language expressions
	\ODPEXAMPLES{
		\emph{tell(has\_participant('EV','O'))} &
		\emph{O} is involved in event \emph{EV} \\
		% % % % %
		\emph{ask(has\_participant('EV',X))} &
		what are the entities involved in \emph{EV}? \\
		% % % % %
		\emph{ask(has\_participant(X,'O'))} &
...
\begin{ODP}{?}
	...
	\begin{tikzpicture}
	    \node[owlclass] (EVT) {?};
	    \node[owlclass,below=0.6cm of EVT] (A) {?};
	    \draw (EVT) edge[relation] node[midway,label=right:?] {} (A);
	\end{tikzpicture}
	}
	%% Example KnowRob language expressions
	\ODPEXAMPLES{
...
\begin{ODP}{??}
	...
	\begin{tikzpicture}
	    ...
	    \draw (B) edge[relation] node[midway,label=right:??] {} (A);
	\end{tikzpicture}
	}
	%% Example KnowRob language expressions
	\ODPEXAMPLES{
...
\begin{ODP}{Transient}
	...
	\ODPDEFINEDIN{SOMA.owl}
	\ODPQUESTION{What sort of object is this? What objects ``went'' in the making of another? Does an object preserve or restore its identity after change?}
	\ODPGRAPHIC{
	\begin{tikzpicture}
 ...
 %};
	    \node[owlclass] (A) {?};
	    \node[owlclass,below=0.6cm of EVT] (B) {?};
	    \draw (B) edge[relation] node[midway,label=right:?] {} (A);
	\end{tikzpicture}
	}
	%% Example KnowRob language expressions
...
\begin{ODP}{satisfies}
	\ODPINTENT{To represent abstractions of situations.}
	\ODPDEFINEDIN{DUL.owl}
	\ODPQUESTION{??}
	\ODPGRAPHIC{
	\begin{tikzpicture}
