...
To deal with occurences, the EASE knowledge base provides the unary \emph{occurs} predicate whose argument is an event entity. It is usually wrapped into a temporal scope to assert the time data of the interval associates to the event. For the tell case, this can be written as: \emph{tell(occurs(Event) during [Begin,End])}.
