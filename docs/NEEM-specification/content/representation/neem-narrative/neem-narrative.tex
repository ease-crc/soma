...
\textbf{(A brief intro about Action taxonomy)}
An action can be defined as an event where at least one agent participates, such that this agent performs a dedicated task, defined by a plan or workflow \todo{Seba: What can the reader understand as workflow ?}, which it executes through the Action. \todo{Seba: There is no mentioning of the difference between Task and Action. Since I am familiar with the ontology I know that there is a difference. However, a person who reads that sentence might get confused since you can understand action and task as synonyms}.
Sometimes intention of an agent is hard to identify even though it may seem obvious that a series of steps required to perform certain activity
which might be subjected to shared cultural assumptions between an agent performing an action and it’s observer. An interpreted action which handles such extra knowledge required to perform certain tasks which can justify an agents behaviour \todo{Seba: I dont understand this sentence.}. Whereas, an observed action, which is simply an observable
fact that an agent has performed during task execution which would in tern liberate us from action interpretation and be helpful to log actions related to manipulation. Bodily motion however deals with information realization which consists movements of an agents body.
In the appendix section, we further describe all three type of actions into subcategories. 
\todo{Seba: From  reading this abstract it was not really clear for me that there are 3 types of actions. It is not clear that body motions and general executions by the robot are seen as different things in the representation, Me personally I am aware of that but again a new reader might be confused. Also I would include some more example since you give a brief introduction but the more interesting and important part is mentioned in the appendix.}

\textbf{(How an Action is related to task)}
An action executes the task which is of special event type. A task can be performed as series of actions according to a plan. Such as cleaning a table would be performed by various set of actions. A plan describes set of action and in which order they needs to be executed in order to perform certain task. \todo{Seba: Where is the real difference between Task and Action ? Can I say also the action grasping executes a task grasping ? Do I only represent high level "actions" such as setting up a table as tasks ? If yes, where is the end of the abstraction ? At the motion level ?} 

\textbf{(task taxonomy)}
A task can be further classfied into three main catagories, physical task, mental task and communication task. A task which requires to execute an action through which an agent has to manipulate representations stored in its own cognition can be categorized as mental task, dreaming, imagining, prospecting, reasoning, retrospecting falls under mental task. Where as physical task requires an agent to perform certain physical activities such as actuating, constructing, modifying physical object, looking at, placing, navigating and perceiving. At last, communication task in which two more more agents shares information. This is used to classify special kind of events that have participants as agents and  social objects. The means of information exchange is physical however the scope of interest here is to  identify which agent communicates and what kind of information it has?
...
An event type which deals with movements of an agent and motions it makes during task execution.\todo{Seba: In the introduction there is mentioned there are only 2 basic elements: classified entities (events and objects) and the relationships. A simple question I have to ask, are tasks also event types? Maybe a real definition of event types would be really useful. } Motion taxonomy includes concepts such as body movement which concentrates on the movements of agents body parts, directed motion which involves a destination and directed path for an agent to follow, an un-directed motion is opposite to directed where agent does not have any particular destination but it is important to know that the agent has moved and motion has occurred, and fluid flow is the process by which fluid moves or has moved from one location to other. \todo{Seba: Thats a very long sentence.} All four concepts are further sub-categorized in appendix section.

\paragraph{Objects} \dots \textbf{(todo: write about 
object taxonomy)}
According to DOLCE, an object participates in an event during its lifetime and has its own spatial location.\todo{Seba: Reference?} SOMA describes objects into several subcategories which includes agent, digital object, feature, physical object, social object and transient. It further includes taxonomy of dispositions which describes the potential way an object can be used and design taxonomy which considers functional, structural and aesthetic aspect of object design.\todo{Seba: An example with using those terms would be nice.}
...
The basic building blocks of NEEMs are the events that occur during the course of an activity\todo{Seba: Is activity a general term for experiments or is it also a definition form the ontology?}. An event is defined as \emph{any physical, social, or mental process, event, or state}. Events have an associated time interval that determines the time at which the event occurs. Time data is represented as unix timestamps using XSD types.
...
\begin{ODP}{Action Execution}
	...
	\ODPQUESTION{Which agent did perform this action? Which actions are performed by this agent?}
	\ODPGRAPHIC{
	\begin{tikzpicture}
	...
	%% Example KnowRob language expressions
	\ODPEXAMPLES{
		\emph{is\_performed\_by('ACT','AG')} &
		\emph{ACT} is performed by \emph{AG} \\
		% % % % %
		\emph{is\_performed\_by('ACT',X)} &
		what are the agents that performed \emph{ACT}? \\
		% % % % %
		\emph{is\_performed\_by(X,'AG')} &
		which actions are performed by \emph{AG}?
	}
\end{ODP}
...
\begin{ODP}{Object Role}
	...
	\ODPEXAMPLES{
		\emph{has\_role('A','B')} &
		\emph{B} is a role of \emph{A} \\
		% % % % %
		\emph{has\_role('A','B') during 'C'} &
		\emph{B} is a role of \emph{A} during the occurrence of \emph{C} \\
		% % % % %
		\emph{has\_role('A','B') during [$t_0$,$t_1$]} &
		\emph{B} is a role of \emph{A} during the timepoints $t_0$ and $t_1$
	}
\end{ODP}
...
Hence, when referring to goals, intentions, etc. we rather employ conceptual classification. \todo{Seba: How the conceptualization of an event looks in the end? Do we have only one classification or can it have multiple classifications if the goal is not clear?}
...
\begin{ODP}{??}
	...
	%% Example KnowRob language expressions
	\ODPEXAMPLES{
		\emph{is\_occurring\_in('C0','Evt0')} &
		\emph{Evt0} is an occurrence that is classified by \emph{C0} \\
		% % % % %
		\emph{is\_executed\_in('Tsk0','Act0')} &
		\emph{Act0} is an action that executes the task \emph{Tsk0}
	}
\end{ODP}
...
\todo{Seba: Do I have to use is\_setting\_for each action and object created in the episode ?}
...
The conceptualization \todo{Seba: can you provide a definition ? Before readining this section I assumed that a synomy for classification}of an episode is an abstraction that refers to concepts instead.
...
by the episode is consistent with the conceptualization given by the description \todo{Seba: Will be description somewhere defined ?}).
