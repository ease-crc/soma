...
\textbf{Task}
An action executes the task which is of special 'EventType'. An event type describes how an Event should be interpreted, executed, expected, seen, etc. A task can be performed as series of actions according to a plan. Such as cleaning a table would be performed by various set of actions. A plan describes set of action and in which order they needs to be executed in order to perform certain task. Here the difference between an action and task needs to be understood, an  \todo{Seba: Where is the real difference between Task and Action ? Can I say also the action grasping executes a task grasping ? Do I only represent high level "actions" such as setting up a table as tasks ? If yes, where is the end of the abstraction ? At the motion level ?} 
...
object taxonomy)}
According to DOLCE, an object participates in an event during its lifetime and has its own spatial location.\todo{Seba: Reference?}  describes objects into several subcategories which includes agent, digital object, feature, physical object, social object and transient. It further includes taxonomy of dispositions which describes the potential way an object can be used and design taxonomy which considers functional, structural and aesthetic aspect of object design.\todo{Seba: An example with using those terms would be nice.}
