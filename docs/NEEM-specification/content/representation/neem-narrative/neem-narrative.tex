...
\begin{ODP}{?}
	...
	\begin{tikzpicture}
	    ...
	    \node[owlclass,below=1.8cm of B] (E) {Force Interaction};
	    \draw (C) edge[relation] node[midway,label=left:classifies] {} (B);
	    \draw (D) edge[relation] node[midway,label=right:classifies] {} (B);
	    \draw (E) edge[relation] node[midway,label=left:xx] {} (C);
	    \draw (E) edge[relation] node[midway,label=right:xx] {} (D);
	\end{tikzpicture}
	}
	%% Example KnowRob language expressions
	\ODPEXAMPLES{
