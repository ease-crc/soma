...
\fcolorbox{easeblue!40}{easeblue!10}{\begin{tabular}{ p{1.8cm} p{3.2cm} }
%\toprule
% {\it\bf Name}                 & \emph{\givenODPNAME} \\
{\noindent\color{easeblue}\it\bf Intent}               & \givenODPINTENT \\
{\noindent\color{easeblue}\it\bf Domains}              & \givenODPDOMAIN \\
{\noindent\color{easeblue}\it\bf Competency Questions} & \givenODPQUESTION \\
...
\ODPEXAMPLES{
	\emph{occurs('EV')} &
	the resource named \emph{EV} is an occurence \\
	% % % % %
	\emph{occurs('EV') during [10,14]} &
	\emph{EV} occurs between the timestamps $10$ and $14$ \\
	% % % % %
	\emph{occurs('EV') since 10} &
	\emph{EV} occurs since the timestamp $10$
}
\end{ODP}
...
Agent participation is handled in a special case, using the property \emph{involvesAgent} which is a sub-property of \emph{hasParticipant}.
...
\ODPDEFINEDIN{DUL.owl}
\ODPQUESTION{
    \emph{Which objects do participate in this event?}
    \emph{Which events do this object participate in?}}
\ODPGRAPHIC{
\begin{tikzpicture}
    \node[owlclass] (EVT) {Event};
    \node[owlclass,below=0.6cm of EVT,xshift=-1.3cm] (OBJ) {Object};
    \node[owlclass,below=0.6cm of EVT,xshift=1.3cm] (A) {Agent};
    \draw (EVT) edge[relation] node[midway,label=left:hasParticipant] {} (OBJ);
    \draw (EVT) edge[relation] node[midway,label=right:involvesAgent] {} (A);
\end{tikzpicture}
}
%% Example KnowRob language expressions
\ODPEXAMPLES{
	\emph{has\_participant('EV','O')} &
	the resource named \emph{O} is involved in the event named \emph{EV} \\
	% % % % %
	\emph{has\_participant('EV',X)} &
	what are the objects that are involved in \emph{EV}? \\
	% % % % %
...
\ODPINTENT{To represent how an entity is composed of others.}
\ODPDOMAIN{?}
\ODPDEFINEDIN{DUL.owl}
\ODPQUESTION{
    \emph{What is this entity composed of?}
    \emph{To which bigger whole does this entity belong?}}
\ODPGRAPHIC{
\begin{tikzpicture}
    \node[owlclass] (E) {Entity};
    \path (E) edge[relation] [loop above] node {hasPart, hasComponent} (E);
\end{tikzpicture}
}
%% Example KnowRob language expressions
\ODPEXAMPLES{
	\emph{has\_component('A','B')} &
	\emph{B} is a proper physical part of \emph{A} \\
	% % % % %
	\emph{has\_part('A','B')} &
	\emph{B} is a part of \emph{A} \\
	% % % % %
	\emph{has\_part('A',X)} &
	What are the parts of \emph{A}? \\
	% % % % %
	\emph{has\_part(X,'A')} &
	What are the entities \emph{A} is part of? \\
}
\end{ODP}

\lipsum[2]

\begin{ODP}{Constituency}
\ODPINTENT{To represent the constituents of a layered structure.}
\ODPDOMAIN{Parts and Collections}
\ODPDEFINEDIN{DUL.owl}
\ODPQUESTION{
    \emph{Which are the constituents of this entity?}
    \emph{What does this entity is constituent of?}}
\ODPGRAPHIC{
\begin{tikzpicture}
    \node[owlclass] (E) {Entity};
    \path (E) edge[relation] [loop above] node {hasConstituent} (E);
\end{tikzpicture}
}
%% Example KnowRob language expressions
\ODPEXAMPLES{
	\emph{has\_constituent('A','B')} &
	\emph{B} is a constituent of \emph{A} \\
}
\end{ODP}
...
\ODPQUESTION{
    \emph{What are the phases of this action?}
    \emph{Is this a phase of some action?}}
\ODPGRAPHIC{
\begin{tikzpicture}
...
\begin{ODP}{Composition}
\ODPINTENT{To represent relations between concepts and entities.}
\ODPDOMAIN{General}
\ODPDEFINEDIN{DUL.owl}
\ODPQUESTION{
    \emph{What concept is assigned to this entity?}
    \emph{Which category does this entity belong to?}}
\ODPGRAPHIC{
\begin{tikzpicture}
    \node[owlclass] (E) {Entity};
    \node[owlclass,below=0.6cm of EVT] (C) {Concept};
    \draw (C) edge[relation] node[midway,label=right:classifies] {} (E);
\end{tikzpicture}
}
%% Example KnowRob language expressions
\ODPEXAMPLES{
	\emph{has\_role('O','R')} &
	\emph{O} is classified by \emph{R} (a role) 
}
\end{ODP}
