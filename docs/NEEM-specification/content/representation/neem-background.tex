...
\lipsum[2]
\todo{Mihai: write about how components of objectsare represented. harmonize with other component section}
...
\begin{ODP}{Mass}
	...
	\ODPEXAMPLES{
		\emph{has\_mass($x$,$y$)} &
		$y$ is the mass of $x$ in kilograms
	}
\end{ODP}
...
\begin{ODP}{Force}
	...
	%% Example KnowRob language expressions
	\ODPEXAMPLES{
		\emph{has\_net\_force($x$,$y$)} \emph{during} $t$ &
		$y$ is the accumulated force, measured in $N$, that acts upon $x$ at time instant $t$
	}
\end{ODP}
