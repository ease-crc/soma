...
\todo{Seba: maybe some more example would be nice. Or refering to the example with the slippery floor. MP: this is also a dangerous thing to do imo unless we also mention that regions are defined to correspond to particular qualities only. DB: well that is how it is defined in DUL, I do not think that the intention is that regions may only be used with qualities, but that qualities are optional and only explicated when individual qualities are in the domain of discourse.}
...
\lipsum[2]
\todo{write about how components of objectsare represented. harmonize with other component section}
