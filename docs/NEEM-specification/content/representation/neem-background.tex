...
space such as \emph{TimeInterval} or \emph{SpaceRegion}.\todo{Seba: Are those concept names? MP: yes}
...
\todo{Seba: maybe some more example would be nice. Or refering to the example with the slippery floor. MP: this is also a dangerous thing to do imo unless we also mention that regions are defined to correspond to particular qualities only.}
...
SOMA defines several concepts to represent qualities relating to an object's appearance: \emph{Color}, \emph{Shape}, \emph{Size}, \emph{Sharpness} etc. A quality belongs to an object, and can take values only from regions of an appropriate type. \todo{MP: hasShape is a specialization of hasQuality for the Shape quality. Do we want to define such properties for all Quality concepts? Not all qualities have restrictions on which regions they take values from. I trust this will be fixed but until then I would not put in the doc something that's not yet in the ontology, so e.g. I wouldn't discuss Size in detail.}
...
A \emph{ShapeRegion} individual can further be classified as a primitive shape (e.g. \emph{BoxShape}, \emph{CylinderShape} etc.) or a \emph{MeshShape}. This finer classification decides what data is attached to the individual. Primitive shapes are described by their geometric parameters, such as height/widhth/length for a box or radius and length for a cylinder. A mesh shape meanwhile has a data property that is a URI to the file that contains the mesh data. To fill in shape data for a shape region, it is necessary to provide data items -- floats or URIs -- and link them via the appropriate data properties to the region individual.

\todo{MP: the ontology defines hasDepth and hasHeight for BoxShape. I assume this is a mistake and we want hasLength and hasHeight.}

\begin{ODP}{Specifying a BoxShape}
	\ODPINTENT{To quantitatively describe a box shape.}
	\ODPDEFINEDIN{SOMA.owl}
	\ODPQUESTION{What quantitative data is available about the shape of an object?}
	\ODPGRAPHIC{
	\begin{tikzpicture}
	    \node[owlclass] (A) {BoxShape};
	    \node[data,below=0.6cm of A,xshift=-1.5cm] (B) {xsd:double};
	    \node[data,below=1.8cm of A] (C) {xsd:double};
	    \node[data,below=0.6cm of A,xshift=1.5cm] (D) {xsd:double};
	    \draw (A) edge[relationxl] node[midway,label=left:hasHeight] {} (B);
	    \draw (A) edge[relationxl,pos=0.8] node[midway,label=right:hasLength] {} (C);
	    \draw (A) edge[relationxl] node[midway,label=right:hasWdith] {} (D);
	\end{tikzpicture}
	}
	%% Example KnowRob language expressions
...
\begin{ODP}{Specifying a CylinderShape}
	\ODPINTENT{To quantitatively describe a cylindric shape.}
	\ODPDEFINEDIN{SOMA.owl}
	\ODPQUESTION{What quantitative data is available about the shape of an object?}
	\ODPGRAPHIC{
	\begin{tikzpicture}
	    \node[owlclass] (A) {CylinderShape};
	    \node[data,below=0.6cm of A,xshift=-1.5cm] (B) {xsd:double};
	    \node[data,below=0.6cm of A,xshift=1.5cm] (C) {xsd:double};
	    \draw (A) edge[relationxl] node[midway,label=left:hasLength] {} (B);
	    \draw (A) edge[relationxl] node[midway,label=right:hasRadius] {} (C);
	\end{tikzpicture}
	}
	%% Example KnowRob language expressions
	\ODPEXAMPLES{\emph{xxx\_xxxxx($x$,$y$)} & \textbf{todo}}
...
\begin{ODP}{Specifying a MeshShape}
	...
	\ODPGRAPHIC{
	\begin{tikzpicture}
	    \node[owlclass] (A) {MeshShape};
	    \node[data,below=0.6cm of A] (B) {URI};
	    \draw (A) edge[relationxl] node[midway,label=left:hasFilePath] {} (B);
	\end{tikzpicture}
	}
	%% Example KnowRob language expressions
	\ODPEXAMPLES{\emph{xxx\_xxxxx($x$,$y$)} & \textbf{todo}}
...
\begin{ODP}{Specifying a SphereShape}
	\ODPINTENT{To link to data about a sphere shape.}
	\ODPDEFINEDIN{SOMA.owl}
	\ODPQUESTION{What quantitative data is available about the shape of an object?}
	\ODPGRAPHIC{
	\begin{tikzpicture}
	    \node[owlclass] (A) {SphereShape};
	    \node[data,below=0.6cm of A] (B) {xsd:double};
	    \draw (A) edge[relationxl] node[midway,label=left:hasRadius] {} (B);
	\end{tikzpicture}
	}
	%% Example KnowRob language expressions
	\ODPEXAMPLES{\emph{xxx\_xxxxx($x$,$y$)} & \textbf{todo}}
...
\begin{ODP}{Joints}
	...
	%% Example KnowRob language expressions
	\ODPEXAMPLES{
		\emph{has\_child\_link($x$,$y$)} &
		$y$ is the child link of joint $x$ \\
		\emph{has\_parent\_link($x$,$y$)} &
		$y$ is the parent link of joint $x$
	}
\end{ODP}
...
\begin{ODP}{Component Chains}
	...
	%% Example KnowRob language expressions
	\ODPEXAMPLES{
		\emph{has\_base\_link($x$,$y$)} &
		$y$ is the first link of $x$ \\
		\emph{has\_end\_link($x$,$y$)} &
		$y$ is one of the last links of $x$
	}
\end{ODP}
...
Depending on wether the joint is either revolute or prismatic, the position is measured in radians or meter respectively.
...
Joint states should be recorded in form of time-series data, and stored as part of the \neemexp.
...
\begin{ODP}{Joint States}
	...
	%% Example KnowRob language expressions
	\ODPEXAMPLES{
		\emph{has\_joint\_position($x$,$y$)} &
		$y$ is the position of joint $x$ \\
		\emph{has\_joint\_velocity($x$,$y$)} &
		$y$ is the velocity of joint $x$ \\
		\emph{has\_joint\_effort($x$,$y$)} &
		$y$ is the effort of joint $x$ \\
	}
\end{ODP}

The movement of a joint may be restricted by physical limits.
This is the case for revolute and prismatic joints.
...
\begin{ODP}{Joint limits}
	...
	%% Example KnowRob language expressions
	\ODPEXAMPLES{
		\emph{has\_joint\_position\_limit($x$,$y$,$z$)} &
		$y$ is the minimum and $z$ the maximum position of joint $x$ \\
		\emph{has\_joint\_velocity\_limit($x$,$y$)} &
		$y$ is the maximum velocity of joint $x$ \\
		\emph{has\_joint\_effort\_limit($x$,$y$)} &
		$y$ is the maximum effort applied in the joint $x$
	}
\end{ODP}
